...
void tlb_add(int index, page_node entry) { 
    // TODO
    // adds entries to the tbl's index
    tlb[index] = entry;
}  

void tlb_remove(int index) { 
    // TODO
    // sets the tbl's index to -1 to indicate it is empty
    tlb[index].npage = -1;
    // checks the tbl's index and sets it to false if it is not present
...
void tlb_hit(size_t& frame, size_t& page, size_t& tlb_hits, int result) {  
    // TODO
    // if frame num is in tlb
    // initialize that it has been hit

    // checks for the index of a tlb entry and returns a matching page
    result = check_tlb(page);

    // checks if result is greater than or equal to 0
    if (0 <= result) {
        // increment hits
        tlb_hits++;
        // setting the frame
        ...
        // sets pg table to true if it is being used
...
void tlb_miss(size_t& frame, size_t& page, size_t& tlb_track) { 
    // TODO
    // if frame num does not exist inside tlb
    // go to the page table and look for corresponding frame num
    // update the tlb with the new page

    // if pages are present in page table
    if (pg_table[page].is_present) {
        // setting the frame
        frame = pg_table[page].frame_num;
        // checks if pg table is in use
        pg_table[page].is_used = true;
        // adds the tlb to track in pg table
...
void fifo_replace_page(size_t& frame) { 
    ...
    // page is set to find the frame within the page table
    size_t page = find_frame_ptable(frame);
    // if page is not empty
    if (page != -1) {
        // checks if pages are present within pg table
        pg_table[page].is_present = false;
    }

    // entry is set to check pages within the table
    size_t entry = check_tlb(page);
...
void lru_replace_page(size_t& frame) { 
    ...
    // getting a page that has not been used recently
    size_t page = get_used_ptable();

    // get frame from unused page entry
    // set frame from physical address
    frame = pg_table[page].frame_num;
    pg_table[page].is_present = false;
    size_t tlb_track = check_tlb(page);
